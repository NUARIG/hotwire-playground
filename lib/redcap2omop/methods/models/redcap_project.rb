      ...
      module RedcapProject
        ...
        module ClassMethods
          ...
          def search_across_fields(search_token, options={})
            ...
            if search_token
              s = where("lower(redcap2omop_redcap_projects.name) like ? OR lower(redcap2omop_redcap_projects.api_token) like ?", "%#{search_token}%", "%#{search_token}%")
            end

            sort = options[:sort_column] + ' ' + options[:sort_direction] + ', redcap2omop_redcap_projects.id ASC'
